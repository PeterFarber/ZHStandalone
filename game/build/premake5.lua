...
function check_enet()
    os.chdir("external")
    if(os.isdir("enet") == false) then
        if(not os.isfile("enet-master.zip")) then
            print("enet not found, downloading from github")
            http.download("https://github.com/lsalzman/enet/archive/refs/heads/master.zip", "enet-master.zip", {
                progress = download_progress,
                headers = { "From: Premake", "Referer: Premake" }
            })
        end
        print("Unzipping enet to " .. os.getcwd())
        zip.extract("enet-master.zip", os.getcwd())
        os.remove("enet-master.zip")
        os.rename("enet-master", "enet")
    end
    os.chdir("../")
end
...
function check_nlohmann_json()
    ...
    if(os.isdir("nlohmann") == false) then
        if(not os.isfile("json.zip")) then
            print("nlohmann/json not found, downloading from github")
            http.download("https://github.com/nlohmann/json/archive/refs/tags/v3.11.3.zip", "json.zip", {
                progress = download_progress,
                headers = { "From: Premake", "Referer: Premake" }
            })
        end
        print("Unzipping nlohmann/json to " .. os.getcwd())
        zip.extract("json.zip", os.getcwd())
        os.remove("json.zip")
        os.rename("json-3.11.3", "nlohmann")
    end
    os.chdir("../")
end
...
if (downloadRaylib) then
    ...
    project (workspaceName)
        ...
        files {enet_dir .. "/*.c"}
        includedirs {enet_dir .. "/include"}

        filter "system:windows"
            removefiles {enet_dir .. "/unix.c"}

        filter "system:not windows"
            removefiles {enet_dir .. "/win32.c"}
        ...
        filter "system:windows"
            defines{"_WIN32"}
            links {"winmm", "gdi32", "opengl32", "ws2_32", "shell32", "user32"}
